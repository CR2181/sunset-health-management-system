...
状态接口接收 `status`、`operatorName` 和可选 `note`。前端提供开始处理、完成任务、标记异常和编辑备注；完成与异常操作均打开备注输入框。每次状态变化记录操作者、前后状态和是否包含备注，不记录备注正文到审计元数据。
...
rehab 仅可读写授权老人；nurse 和 family 接收删减后的只读摘要；visitor 返回 403。
